...
function updateUtensilTagsListeners () {
    const utensilsTagsItems = document.getElementsByClassName('utensil-tag-item-remove')

    for (let i = 0; i < utensilsTagsItems.length; i++) {
        utensilsTagsItems[i].addEventListener('click', (event) => {
            const utensilName = utensilsTagsItems[i].parentNode.childNodes[0].textContent
...
function updateIngredientTagsListeners () {
    const ingredientTagsItems = document.getElementsByClassName('ingredient-tag-item-remove')

    for (let i = 0; i < ingredientTagsItems.length; i++) {
        ingredientTagsItems[i].addEventListener('click', (event) => {
            const ingredientName = ingredientTagsItems[i].parentNode.childNodes[0].textContent
...
function updateApplianceTagsListeners () {
    const applianceTagsItems = document.getElementsByClassName('appliance-tag-item-remove')

    for (let i = 0; i < applianceTagsItems.length; i++) {
        applianceTagsItems[i].addEventListener('click', (event) => {
            const applianceName = applianceTagsItems[i].parentNode.childNodes[0].textContent
